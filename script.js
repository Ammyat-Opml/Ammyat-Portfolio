...
(function () {
  ...
  // Mark that JS is active. CSS uses this so that if the script ever fails
  // to load, the reveal elements stay visible instead of the page looking blank.
  ...
  /* --------------------------------------------
     Redesign: smooth in-page scroll, scroll-reveal,
     sticky-bar state, and hero mouse parallax.
  -------------------------------------------- */
  var reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  // Smooth scroll for in-page anchors (brand, nav, hero CTAs, scroll cue)
  ...
  // Sticky top bar: solid backdrop once the page scrolls a little
  ...
  // Hero mouse parallax (only on motion-allowed, pointer-capable devices)
  ...
  // Site-wide starfall — falling stars + a few static twinkles behind all content.
  // Generated here so positions/speeds vary; skipped under reduced motion.
  var starField = document.querySelector('.starfield');
  if (starField && !reduceMotion) {
    var vw = window.innerWidth;
    var fallCount = vw < 680 ? 20 : (vw < 1100 ? 32 : 46);
    var twinkleCount = vw < 680 ? 12 : 22;
    ...
    for (var i = 0; i < fallCount; i++) {
      ...
      s.style.setProperty('--l', rand(0, 100).toFixed(2) + '%');
      s.style.setProperty('--len', rand(36, 90).toFixed(0) + 'px');
      s.style.setProperty('--t', rand(4.5, 9.5).toFixed(2) + 's');
      s.style.setProperty('--d', (-rand(0, 9.5)).toFixed(2) + 's');   // negative: start mid-fall
      s.style.setProperty('--dx', rand(-34, 18).toFixed(0) + 'px');
      s.style.setProperty('--o', rand(0.45, 0.9).toFixed(2));
      s.style.setProperty('--star-col', accent ? '#ff8c66' : '#cdd6e6');
      frag.appendChild(s);
    }

    for (var j = 0; j < twinkleCount; j++) {
      var d = document.createElement('span');
      d.className = 'twinkle';
      var sz = rand(1, 2.6).toFixed(1);
      d.style.left = rand(0, 100).toFixed(2) + '%';
      d.style.top = rand(0, 92).toFixed(2) + '%';
      d.style.width = sz + 'px';
      d.style.height = sz + 'px';
      d.style.setProperty('--tt', rand(2.4, 5).toFixed(2) + 's');
      d.style.setProperty('--td', (-rand(0, 5)).toFixed(2) + 's');
      if (Math.random() < 0.2) d.style.background = '#ff8c66';
      frag.appendChild(d);
    }
